...
class AgoraRtmClient {
  ...
  /// Callback to the caller: occurs when the caller accepts the call
  /// invitation.
  void Function(AgoraRtmLocalInvitation invite)? onLocalInvitationAccepted;

  /// Callback to the caller: occurs when the caller declines the call
  /// invitation.
  ...
  /// Callback to the caller: occurs when the callee accepts the call
  /// invitation.
  void Function(AgoraRtmRemoteInvitation invite)? onRemoteInvitationAccepted;

  /// Callback to the caller: occurs when the callee declines the call
  /// invitation.
  ...
  EventChannel _addEventChannel(name) => EventChannel(name);
  ...
  Future<dynamic> _callNative(String methodName, dynamic arguments) =>
      AgoraRtmPlugin.callMethodForClient(
  ...
  ///
  /// The string length of userId must be less than 64 bytes with the following
  /// character scope:
  ...
  /// If you log in with the same user ID from a different instance, you will
  /// be kicked out of your previous login and removed from previously joined
  /// channels.
  ...
  ///
  /// channelId is the unique channel name of the Agora RTM session. The string
  /// length must not exceed 64 bytes with the following character scope:
  /// - The 26 lowercase English letters: a to z
  /// - The 26 uppercase English letters: A to Z
  /// - The 10 numbers: 0 to 9
  /// - Space
  ...
  /// Gets all attributes of a specified channel.
  Future<List<AgoraRtmChannelCount>> getChannelMemberCount(
      String channelId) async {
    final res =
        await _callNative("getChannelMemberCount", {'channelId': channelId});
    if (res["errorCode"] != 0) {
      throw AgoraRtmClientException(
          "getChannelAttributes failed errorCode:${res['errorCode']}",
          res['errorCode']);
    ...
    return List<Map<dynamic, dynamic>>.from(res["attributes"])
        .map((attr) => AgoraRtmChannelCount.fromJson(attr))
        .toList();
  }
}
